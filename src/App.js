...
function NavBar({ active, setActive }) {
  ...
  return (
    <>
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, padding: "0 24px", height: 68, display: "flex", alignItems: "center", justifyContent: "space-between", background: scrolled || menuOpen ? "rgba(22,20,18,0.97)" : "transparent", backdropFilter: scrolled ? "blur(20px)" : "none", borderBottom: scrolled ? "1px solid rgba(255,255,255,0.06)" : "none", transition: "all 0.4s ease" }}>
        <img src={logo} alt="DJ Appz" onClick={() => nav("home")} style={{ height: 52, objectFit: "contain", filter: "invert(1)", cursor: "pointer", opacity: 0.92 }} />
        {isMobile ? (
          <button onClick={() => setMenuOpen(o => !o)} style={{ background: "none", border: "none", cursor: "pointer", color: "#fff", fontSize: 22, padding: 4 }}>
            {menuOpen ? "✕" : "☰"}
          </button>
        ) : (
          <div style={{ display: "flex", gap: 36 }}>
            {links.map(l => (
              <button key={l} onClick={() => nav(l)} style={{ background: "none", border: "none", cursor: "pointer", fontFamily: "'Outfit', sans-serif", fontSize: 10, letterSpacing: 2.5, textTransform: "uppercase", color: active === l ? "#fff" : "rgba(255,255,255,0.35)", transition: "color 0.2s", padding: 0 }}>{l}</button>
            ))}
          </div>
        )}
      </nav>
      ...
      {isMobile && menuOpen && (
        <div style={{ position: "fixed", top: 68, left: 0, right: 0, zIndex: 99, background: "rgba(22,20,18,0.98)", borderBottom: "1px solid rgba(255,255,255,0.08)", padding: "16px 0" }}>
          {links.map(l => (
            <button key={l} onClick={() => nav(l)} style={{ display: "block", width: "100%", background: "none", border: "none", cursor: "pointer", fontFamily: "'Outfit', sans-serif", fontSize: 13, letterSpacing: 2.5, textTransform: "uppercase", color: active === l ? "#fff" : "rgba(255,255,255,0.45)", padding: "14px 24px", textAlign: "left" }}>{l}</button>
          ))}
        </div>
      )}
    </>
  ...
  return (
    <section style={{ minHeight: "calc(var(--vh, 1vh) * 100)", display: "flex", alignItems: "center", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "absolute", inset: 0 }}>
        <img src={heroPhoto} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: isMobile ? "center top" : "right top" }} />
        {/* Dark overlay — stronger on mobile for text readability */}
        <div style={{ position: "absolute", inset: 0, background: isMobile ? "linear-gradient(to bottom, rgba(22,20,18,0.75) 0%, rgba(22,20,18,0.85) 60%, rgba(22,20,18,0.98) 100%)" : "linear-gradient(to right, rgba(22,20,18,1) 0%, rgba(22,20,18,0.85) 45%, rgba(22,20,18,0.2) 100%)" }} />
      </div>
      ...
      <div style={{ position: "relative", zIndex: 2, padding: isMobile ? "120px 24px 80px" : "120px 48px 80px", maxWidth: 640, width: "100%" }}>
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", opacity: 0, animation: "fadeUp 0.8s 0.8s forwards" }}>
          <button onClick={onBook} style={{ background: "#fff", color: "#161412", border: "none", borderRadius: 2, padding: "14px 36px", fontFamily: "'Outfit', sans-serif", fontSize: 10, letterSpacing: 2.5, textTransform: "uppercase", cursor: "pointer", fontWeight: 500 }}>Book Now</button>
          <button onClick={() => document.getElementById("about-section")?.scrollIntoView({ behavior: "smooth" })} style={{ background: "none", color: "rgba(255,255,255,0.5)", border: "1px solid rgba(255,255,255,0.2)", borderRadius: 2, padding: "14px 36px", fontFamily: "'Outfit', sans-serif", fontSize: 10, letterSpacing: 2.5, textTransform: "uppercase", cursor: "pointer" }}>About</button>
        </div>
      </div>
    </section>
  );
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: isMobile ? 48 : 80, alignItems: "center" }}>
        <Reveal>
          <div style={{ position: "relative" }}>
            <img src={mixerPhoto} alt="DJ Appz at the decks" style={{ width: "100%", aspectRatio: isMobile ? "4/3" : "3/4", objectFit: "cover", objectPosition: "center top", borderRadius: 2, display: "block" }} />
            ...
            <div style={{ position: "absolute", bottom: 20, left: 20, background: "rgba(22,20,18,0.9)", backdropFilter: "blur(12px)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 2, padding: "14px 18px" }}>
              <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 26, color: "#fff", fontStyle: "italic", lineHeight: 1 }}>200+</div>
        ...
        <Reveal delay={isMobile ? 0 : 0.2}>
          <div>
            ...
            <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: "clamp(32px, 4vw, 48px)", color: "#fff", margin: "0 0 24px", lineHeight: 1.15 }}>The Sound of<br /><em>Every Room</em></h2>
            <p style={{ fontFamily: "'Outfit', sans-serif", fontSize: 14, color: "rgba(255,255,255,0.5)", lineHeight: 1.9, margin: "0 0 32px" }}>{DJ.bio}</p>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 36 }}>
              {GENRES.map(g => <span key={g} style={{ fontFamily: "'Outfit', sans-serif", fontSize: 10, letterSpacing: 2, color: "rgba(180,140,80,0.7)", border: "1px solid rgba(180,140,80,0.2)", borderRadius: 1, padding: "5px 12px" }}>{g}</span>)}
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 1, background: "rgba(255,255,255,0.06)" }}>
              {[["4+", "Years"], ["4.9★", "Rated"], ["UK", "& Beyond"]].map(([val, label]) => (
                <div key={label} style={{ background: "#1e1c1a", padding: "20px 16px" }}>
...
function MixesSection() {
  const isMobile = useIsMobile();
  return (
    <section style={{ padding: isMobile ? "80px 24px" : "120px 48px", background: "rgba(0,0,0,0.2)" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        <Reveal>
          <div style={{ marginBottom: 48 }}>
            ...
            <p style={{ fontFamily: "'Outfit', sans-serif", fontSize: 13, color: "rgba(255,255,255,0.35)", lineHeight: 1.7, maxWidth: 480 }}>A selection of recorded sets — the best way to get a feel for the vibe before you book.</p>
          </div>
        </Reveal>
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(2, 1fr)", gap: isMobile ? 16 : 1, background: isMobile ? "transparent" : "rgba(255,255,255,0.06)" }}>
          {MIXES.map((mix, i) => (
            <Reveal key={mix.id} delay={i * 0.15}>
              <div style={{ background: "#1e1c1a", padding: isMobile ? "24px 20px" : "32px 32px 28px", borderRadius: isMobile ? 4 : 0 }}>
                <div style={{ display: "flex", alignItems: isMobile ? "flex-start" : "center", justifyContent: "space-between", marginBottom: 18, flexDirection: isMobile ? "column" : "row", gap: isMobile ? 8 : 0 }}>
                  <div>
                    <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: 18, color: "#fff", margin: "0 0 4px", fontWeight: 400 }}>{mix.title}</h3>
                    <p style={{ fontFamily: "'Outfit', sans-serif", fontSize: 11, color: "rgba(255,255,255,0.3)", letterSpacing: 1.5, margin: 0 }}>{mix.subtitle}</p>
                  </div>
                  <span style={{ fontFamily: "'Outfit', sans-serif", fontSize: 9, letterSpacing: 2, color: "rgba(180,140,80,0.7)", border: "1px solid rgba(180,140,80,0.2)", padding: "4px 10px", borderRadius: 1, whiteSpace: "nowrap" }}>{mix.tag}</span>
                </div>
                ...
                <div style={{ marginTop: 12, textAlign: "right" }}>
                  <a href="https://www.mixcloud.com/DJAppz/" target="_blank" rel="noopener noreferrer" style={{ fontFamily: "'Outfit', sans-serif", fontSize: 10, letterSpacing: 2, color: "rgba(255,255,255,0.2)", textDecoration: "none" }}>MORE ON MIXCLOUD →</a>
                </div>
              </div>
            </Reveal>
          ))}
  ...
  return (
    <section style={{ padding: isMobile ? "80px 24px" : "120px 48px" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(2, 1fr)", gap: isMobile ? 12 : 1, background: isMobile ? "transparent" : "rgba(255,255,255,0.06)" }}>
          {SERVICES.map((s, i) => (
            <Reveal key={s.id} delay={i * 0.1}>
              <div onClick={() => onBook(s.id)} style={{ background: "#1e1c1a", padding: isMobile ? "28px 24px" : "44px 40px", cursor: "pointer", transition: "background 0.3s", borderRadius: isMobile ? 4 : 0 }} onMouseEnter={e => e.currentTarget.style.background = "#252220"} onMouseLeave={e => e.currentTarget.style.background = "#1e1c1a"}>
                <div style={{ fontSize: 28, marginBottom: 12, color: "rgba(255,255,255,0.15)" }}>{s.icon}</div>
                <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: 22, color: "#fff", margin: "0 0 8px", fontWeight: 400 }}>{s.title}</h3>
                <p style={{ fontFamily: "'Outfit', sans-serif", fontSize: 11, color: "rgba(255,255,255,0.3)", letterSpacing: 1.5, margin: "0 0 24px" }}>{s.sub}</p>
...
function TestimonialsSection() {
  ...
  return (
    <section style={{ padding: isMobile ? "80px 24px" : "120px 48px", background: "rgba(0,0,0,0.15)" }}>
      <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
        <Reveal>
          <div style={{ fontFamily: "'Outfit', sans-serif", fontSize: 10, letterSpacing: 4, color: "rgba(180,140,80,0.7)", marginBottom: 40 }}>KIND WORDS</div>
          <div style={{ minHeight: 160 }}>
            <p key={active} style={{ fontFamily: "'Playfair Display', serif", fontSize: "clamp(16px, 2.5vw, 24px)", color: "rgba(255,255,255,0.85)", lineHeight: 1.65, margin: "0 0 28px", fontStyle: "italic", animation: "fadeUp 0.5s ease" }}>"{t.quote}"</p>
            <div style={{ fontFamily: "'Outfit', sans-serif", fontSize: 11, color: "rgba(255,255,255,0.3)", letterSpacing: 2 }}>{t.name} — {t.event}</div>
          </div>
          <div style={{ display: "flex", justifyContent: "center", gap: 8, marginTop: 36 }}>
            {TESTIMONIALS.map((_, i) => <button key={i} onClick={() => setActive(i)} style={{ width: i === active ? 28 : 6, height: 2, background: i === active ? "rgba(180,140,80,0.8)" : "rgba(255,255,255,0.15)", border: "none", cursor: "pointer", transition: "all 0.3s", padding: 0, borderRadius: 1 }} />)}
          </div>
        </Reveal>
      </div>
    </section>
    ...
    <section style={{ padding: isMobile ? "80px 24px" : "120px 48px" }}>
      <div style={{ maxWidth: 700, margin: "0 auto" }}>
        <Reveal>
          ...
          <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: "clamp(28px, 4vw, 40px)", color: "#fff", margin: "0 0 48px" }}>Common Questions</h2>
        </Reveal>
        {FAQS.map((f, i) => (
          <Reveal key={i} delay={i * 0.05}>
            <div style={{ borderTop: "1px solid rgba(255,255,255,0.07)" }}>
              <button onClick={() => setOpen(open === i ? null : i)} style={{ width: "100%", background: "none", border: "none", padding: "20px 0", display: "flex", justifyContent: "space-between", alignItems: "center", cursor: "pointer", textAlign: "left", gap: 16 }}>
                <span style={{ fontFamily: "'Outfit', sans-serif", fontSize: 13, color: "rgba(255,255,255,0.75)" }}>{f.q}</span>
                <span style={{ color: "rgba(180,140,80,0.6)", fontSize: 20, transform: open === i ? "rotate(45deg)" : "none", transition: "transform 0.25s", flexShrink: 0, lineHeight: 1 }}>+</span>
              </button>
              <div style={{ overflow: "hidden", maxHeight: open === i ? 300 : 0, transition: "max-height 0.35s ease", paddingBottom: open === i ? 20 : 0 }}>
                <p style={{ fontFamily: "'Outfit', sans-serif", fontSize: 13, color: "rgba(255,255,255,0.4)", lineHeight: 1.8, margin: 0 }}>{f.a}</p>
              </div>
            </div>
          </Reveal>
        ))}
...
function DatePicker({ value, onChange, error }) {
  ...
  return (
    <div style={{ position: "relative" }}>
      <button type="button" onClick={() => setOpen(o => !o)} style={{ width: "100%", background: "none", border: "none", borderBottom: `1px solid ${error ? "rgba(200,80,80,0.6)" : "rgba(255,255,255,0.12)"}`, padding: "12px 0", fontFamily: "'Outfit', sans-serif", fontSize: 13, color: value ? "#fff" : "rgba(255,255,255,0.25)", cursor: "pointer", textAlign: "left", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <span>{value || "Select a date…"}</span>
        <span style={{ color: "rgba(180,140,80,0.6)", fontSize: 11 }}>📅</span>
      </button>
      {open && (
        <div style={{ position: "absolute", top: "calc(100% + 8px)", left: isMobile ? "50%" : 0, transform: isMobile ? "translateX(-50%)" : "none", zIndex: 300, background: "#1e1c1a", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 4, padding: 16, width: isMobile ? "calc(100vw - 48px)" : 300, boxShadow: "0 20px 60px rgba(0,0,0,0.8)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            <button type="button" onClick={prevMonth} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.4)", cursor: "pointer", fontSize: 18, padding: "4px 10px" }}>‹</button>
            <span style={{ fontFamily: "'Playfair Display', serif", fontSize: 14, color: "#fff", fontStyle: "italic" }}>{MONTHS[viewMonth]} {viewYear}</span>
            <button type="button" onClick={nextMonth} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.4)", cursor: "pointer", fontSize: 18, padding: "4px 10px" }}>›</button>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 2, marginBottom: 6 }}>
            {DAYS.map(d => <div key={d} style={{ fontFamily: "'Outfit', sans-serif", fontSize: 9, letterSpacing: 1, color: "rgba(255,255,255,0.2)", textAlign: "center", padding: "4px 0" }}>{d}</div>)}
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 2 }}>
            ...
            {Array.from({ length: daysInMonth }).map((_, i) => {
              ...
              return (
                <button type="button" key={day} onClick={() => !past && selectDate(day)} style={{ background: sel ? "rgba(180,140,80,0.9)" : tod ? "rgba(255,255,255,0.08)" : "none", border: tod && !sel ? "1px solid rgba(180,140,80,0.3)" : "1px solid transparent", borderRadius: 2, padding: "8px 0", fontFamily: "'Outfit', sans-serif", fontSize: 11, color: past ? "rgba(255,255,255,0.15)" : sel ? "#161412" : "#fff", cursor: past ? "not-allowed" : "pointer", textAlign: "center", fontWeight: sel ? 600 : 400 }}
                  onMouseEnter={e => { if (!past && !sel) e.currentTarget.style.background = "rgba(180,140,80,0.15)"; }}
          ...
          <div style={{ marginTop: 12, textAlign: "right" }}>
            <button type="button" onClick={() => setOpen(false)} style={{ background: "none", border: "none", fontFamily: "'Outfit', sans-serif", fontSize: 10, letterSpacing: 2, color: "rgba(255,255,255,0.25)", cursor: "pointer" }}>CLOSE</button>
          </div>
        </div>
      )}
    </div>
...
function BookingForm({ prefill, onSuccess }) {
  ...
  const iStyle = (k) => ({ width: "100%", background: "none", border: "none", borderBottom: `1px solid ${errors[k] ? "rgba(200,80,80,0.6)" : "rgba(255,255,255,0.15)"}`, padding: "12px 0", fontFamily: "'Outfit', sans-serif", fontSize: 13, color: "#fff", outline: "none", boxSizing: "border-box", transition: "border-color 0.2s" });
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: isMobile ? 20 : 24 }}>
        ...
        <div>
          ...
          <select value={f.eventType} onChange={e => set("eventType", e.target.value)} style={{ ...iStyle("eventType"), color: f.eventType ? "#fff" : "rgba(255,255,255,0.3)", cursor: "pointer" }}>
            ...
            {SERVICES.map(s => <option key={s.id} value={s.title} style={{ background: "#1e1c1a" }}>{s.title}</option>)}
          </select>
      ...
      <button onClick={submit} disabled={sending} style={{ alignSelf: isMobile ? "stretch" : "flex-start", background: sending ? "rgba(255,255,255,0.1)" : "#fff", color: "#161412", border: "none", padding: "16px 48px", fontFamily: "'Outfit', sans-serif", fontSize: 10, letterSpacing: 3, textTransform: "uppercase", cursor: sending ? "not-allowed" : "pointer", borderRadius: 1, fontWeight: 500, textAlign: "center" }}>
    ...
    <section style={{ padding: isMobile ? "80px 24px" : "120px 48px" }}>
      <div style={{ maxWidth: 700, margin: "0 auto" }}>
        <Reveal>
          ...
          <p style={{ fontFamily: "'Outfit', sans-serif", fontSize: 13, color: "rgba(255,255,255,0.35)", margin: "0 0 48px", lineHeight: 1.7 }}>Fill in the form and I'll be in touch within 24 hours to discuss your event.</p>
        </Reveal>
  ...
  return (
    <section style={{ padding: isMobile ? "80px 24px" : "120px 48px", minHeight: "80vh", display: "flex", alignItems: "center" }}>
      <div style={{ maxWidth: 580, margin: "0 auto", textAlign: "center", opacity: show ? 1 : 0, transform: show ? "none" : "translateY(24px)", transition: "opacity 0.7s ease, transform 0.7s ease" }}>
        <div style={{ width: 80, height: 80, borderRadius: "50%", background: "rgba(180,140,80,0.1)", border: "1px solid rgba(180,140,80,0.4)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 32, color: "rgba(180,140,80,0.9)", margin: "0 auto 28px", animation: "pulseRing 2s ease-in-out infinite" }}>✓</div>
        ...
        <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: "clamp(26px, 4vw, 38px)", color: "#fff", margin: "0 0 20px", lineHeight: 1.2 }}>You're on our <em>radar</em>, {form.name.split(" ")[0]}.</h2>
        <div style={{ background: "rgba(180,140,80,0.06)", border: "1px solid rgba(180,140,80,0.2)", borderRadius: 4, padding: "20px 24px", marginBottom: 20, textAlign: "left" }}>
          <p style={{ fontFamily: "'Outfit', sans-serif", fontSize: 13, color: "rgba(255,255,255,0.6)", lineHeight: 1.9, margin: 0 }}>
            Your booking enquiry has been successfully submitted. A member of the <strong style={{ color: "#fff" }}>DJ Appz</strong> team will be in touch at <strong style={{ color: "rgba(180,140,80,0.9)" }}>{form.email}</strong> within <strong style={{ color: "#fff" }}>24 hours</strong>. A confirmation has also been sent to your email.
          </p>
        </div>
        <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 4, padding: "20px 24px", textAlign: "left", marginBottom: 20 }}>
          <div style={{ fontFamily: "'Outfit', sans-serif", fontSize: 9, letterSpacing: 3, color: "rgba(255,255,255,0.2)", marginBottom: 14 }}>YOUR BOOKING SUMMARY</div>
        ...
        <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 4, padding: "20px 24px", textAlign: "left", marginBottom: 28 }}>
          <div style={{ fontFamily: "'Outfit', sans-serif", fontSize: 9, letterSpacing: 3, color: "rgba(255,255,255,0.2)", marginBottom: 16 }}>WHAT HAPPENS NEXT</div>
        ...
        <p style={{ fontFamily: "'Outfit', sans-serif", fontSize: 11, color: "rgba(255,255,255,0.2)", letterSpacing: 1 }}>Direct line: <span style={{ color: "rgba(180,140,80,0.6)" }}>{DJ.email}</span></p>
      </div>
    </section>
  );
}
...
function FooterBar() {
  ...
  return (
    <footer style={{ borderTop: "1px solid rgba(255,255,255,0.06)", padding: isMobile ? "32px 24px" : "40px 48px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 16 }}>
      <span style={{ fontFamily: "'Playfair Display', serif", fontSize: 14, color: "rgba(255,255,255,0.25)", fontStyle: "italic", letterSpacing: 2 }}>DJ Appz</span>
      <div style={{ display: "flex", gap: isMobile ? 16 : 28, flexWrap: "wrap" }}>
        {[DJ.instagram, DJ.email, DJ.location].map(l => <span key={l} style={{ fontFamily: "'Outfit', sans-serif", fontSize: 10, letterSpacing: 1.5, color: "rgba(255,255,255,0.2)" }}>{l}</span>)}
      </div>
...
function ChatBot({ onOpenBook }) {
  ...
  return (
    <>
      <button onClick={() => setOpen(o => !o)} style={{ position: "fixed", bottom: 24, right: 24, zIndex: 200, width: 52, height: 52, borderRadius: "50%", background: "#fff", border: "none", cursor: "pointer", fontSize: 18, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 32px rgba(0,0,0,0.5)", transition: "transform 0.2s" }}>
      ...
      {open && (
        <div style={{ position: "fixed", bottom: 88, right: 24, zIndex: 199, width: "min(340px, calc(100vw - 48px))", maxHeight: 500, background: "#1a1816", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 4, display: "flex", flexDirection: "column", boxShadow: "0 24px 64px rgba(0,0,0,0.8)", animation: "fadeUp 0.2s ease" }}>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "14px 14px 8px", display: "flex", flexDirection: "column", gap: 10 }}>
            {msgs.map((m, i) => (
              <div key={i} style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start" }}>
                <div style={{ maxWidth: "85%", background: m.role === "user" ? "#fff" : "rgba(255,255,255,0.07)", color: m.role === "user" ? "#161412" : "rgba(255,255,255,0.75)", padding: "10px 13px", borderRadius: 2, fontFamily: "'Outfit', sans-serif", fontSize: 12, lineHeight: 1.65 }}>{m.content}</div>
              </div>
            ))}
            {loading && <div style={{ display: "flex", gap: 5, padding: "10px 13px" }}>{[0, 0.2, 0.4].map((d, i) => <div key={i} style={{ width: 5, height: 5, borderRadius: "50%", background: "rgba(255,255,255,0.3)", animation: `blink 1s ${d}s infinite` }} />)}</div>}
            <div ref={endRef} />
          </div>
          <div style={{ padding: "8px 12px 0", display: "flex", gap: 6, overflowX: "auto" }}>
            {quickReplies.map((q, i) => <button key={i} onClick={() => send(q)} style={{ background: "none", border: "1px solid rgba(180,140,80,0.25)", borderRadius: 1, padding: "5px 10px", color: "rgba(180,140,80,0.7)", fontFamily: "'Outfit', sans-serif", fontSize: 10, letterSpacing: 1, whiteSpace: "nowrap", cursor: "pointer", flexShrink: 0 }}>{q}</button>)}
          </div>
    ...
    <>
      <style>{`
        ...
        * { max-width: 100%; }
        ...
        input:focus { border-bottom-color: rgba(180,140,80,0.5) !important; }
        textarea:focus { border-bottom-color: rgba(180,140,80,0.5) !important; outline: none; }
      `}</style>
      ...
      <div style={{ minHeight: "100vh" }}>
            ...
            <section style={{ padding: isMobile ? "80px 24px" : "100px 48px", textAlign: "center", borderTop: "1px solid rgba(255,255,255,0.06)", position: "relative", overflow: "hidden" }}>
              ...
              <Reveal>
                <div style={{ position: "relative" }}>
                  ...
                  <p style={{ fontFamily: "'Outfit', sans-serif", fontSize: 13, color: "rgba(255,255,255,0.3)", margin: "0 0 32px" }}>Let's talk about your event.</p>
                  <button onClick={() => goBook()} style={{ background: "#fff", color: "#161412", border: "none", padding: "15px 48px", fontFamily: "'Outfit', sans-serif", fontSize: 10, letterSpacing: 3, textTransform: "uppercase", cursor: "pointer", borderRadius: 1, fontWeight: 500 }}>Get in Touch</button>
                </div>
              </Reveal>
            </section>
          </>
        ...
        {section === "book" && (<><div style={{ paddingTop: 68 }} />{submitted ? <SuccessSection form={submitted} /> : <BookingSection prefill={bookPrefill} onSuccess={setSubmitted} />}</>)}
