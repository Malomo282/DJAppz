...
Never invent details. If unsure, direct them to dj-appz@outlook.com.`;

  try {
    const res = await fetch("/api/chat", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ system: sys, messages, max_tokens: 500 }),
    });

    // Handle rate limiting gracefully
    if (res.status === 429) {
      const data = await res.json();
      return data.message || "You've sent a lot of messages — please wait a few minutes before trying again.";
    }

    if (!res.ok) {
      return `Something went wrong. Please email us directly at dj-appz@outlook.com and we'll get back to you shortly.`;
    }

    const d = await res.json();
    return d.content?.[0]?.text || `Drop me a message at dj-appz@outlook.com and I'll get back to you shortly.`;

  } catch (err) {
    return `Drop me a message at dj-appz@outlook.com and I'll get back to you shortly.`;
  }
}
  ...
  return (
    <section style={{ minHeight: "calc(var(--vh, 1vh) * 100)", display: "flex", alignItems: "center", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "relative", zIndex: 2, padding: isMobile ? "120px 24px 80px" : "120px 48px 80px", maxWidth: 640, width: "100%" }}>
        ...
        <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: isMobile ? "56px" : "clamp(56px, 7vw, 80px)", lineHeight: 0.9, margin: "0 0 28px", color: "#1a1815", opacity: 0, animation: "fadeUp 0.8s 0.4s forwards" }}>
